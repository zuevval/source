...
int count (int vers, Node *trees){ @\label{line:count}@
	//returns the number of nodes (excluding superroot)
    int stackTop, res = 0;
...
void newVersPop(Node * trees, int vers, int data){ @\label{line:newVersPop}@
...
void newVersPush(Node * trees, int vers, int data){ @\label{line:newVersPush}@
...
int findNode (Node * t, int data){ @\label{line:findNode}@
...
void removeVers(int vers, Node *trees){ @\label{line:removeVers}@
    int stackTop;
    Node ** stack = (Node **)malloc(maxdepth*sizeof(Node*));
    Node * t = trees + vers;
    stackTop = 0;
