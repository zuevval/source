...
rowFilter <- (cr < min.callrate) & (maf > min.maf) # should be colFilter, but checkpoint is saved...
...
```
```{r}
plot(x[ant$sex == "F"], y[ant$sex == "F"], col = "blue")
plot(x[ant$sex == "M"], y[ant$sex == "M"], col = "red")
```
...
snpset <- as.character(sample(unlist(snp.subset), 600))
...
- по CallRate (>0.95) и MAF (>0.05) отфильтровали суммарно 658 SNP (пункт 5)
- по проценту генотипированных SNP (<98%) не отфильтровали никого из индивидов (все качественные)
- по HWE (< 10^{-10} в контрольной группе, < 10^{-5} в исследуемой) прошли все SNP
- по LD для PCA отобраны 19.440 SNP, остальные считаются зависимыми
- по результатам анализа главных компонент отфильтрованы 6 образцов (4 контрольных, 2 из исследуемой группы)
- по результатам анализа родственных связей (kinship > 1/8) родственников не выявили
