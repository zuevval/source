...
conf.s2 <- var(dd$y)
conf.xxt_inv <- solve(order2.x %*% t(order2.x))
...
```{r}
hypot.c1 <- rbind(c(1, 0), c(0, 1))
hypot.c2 <- matrix(c(1, 0), nrow = 2)
order1.x <- order2.x %>% head(-1)
hypot.xxt_inv <- solve(order1.x %*% t(order1.x))
hypot.f_stats <- list("dependent" = hypot.c1, "independent" = hypot.c2) %>%
  lapply(function(c = ? matrix) {
    hypot.beta <- t(c) %*% hypot.xxt_inv %*% c
    psi <- t(c) %*% order1.regression$coef
