...
q <- 2 # the length of the parameter vector (we look at two parameters)
bonferroni.quantile <- qf(p = 1 - param.alpha / dd.n, df1 = q, df2 = dd.n - dd.r)

conf.c <- rbind(c(0, 0), c(1, 0), c(0, 1))
conf.s2 <- var(dd$y)
conf.gamma <- conf.s2 * t(conf.c) %*% solve(order2.x %*% t(order2.x)) %*% conf.c

order2.tested_coefs <- t(conf.c) %*% order2.regression$coef
order2.confdelta <- bonferroni.quantile * sqrt(diag(conf.gamma))
...
confint(order2.linregr, parm = c(2, 3), level = 1 - param.alpha)
```
## (e) linear dependency / independency hypotheses
```{r}
confint(lm(dd$y ~ dd$x), parm = 2, level = 1 - param.alpha)
```
## (f) AIC, BIC
