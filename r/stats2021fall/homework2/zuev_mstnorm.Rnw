...
Under the null hypothesis, $ \sum_i x_i \sim \mathcal N(na_0, n\sigma^2) \Rightarrow  C_\alpha$ is the $\alpha$-quantile of this distribution

<<cache=F>>=
c_alpha2 <- qnorm(mean=x.n*x.mean,sd=sqrt(x.n*x.s2), p=param.alpha2) %>% abs %>% log
log_ratio <- (param.alpha1 - param.alpha0) * sum(x) / (2 * param.sigma0^2) %>% abs
print(log_ratio <= c_alpha2)
@

The test suggests that we prefer $ H_0 $ over $ H_1 $.

The power of the test $ W = \exp((\ln W)) $
