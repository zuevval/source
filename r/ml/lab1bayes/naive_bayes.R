...
### Naive Bayesian (categorial data)
...
n <- dim(A_raw)[1] # number of rows in A = number of data samples
...
bayes_tic_tac_toe <- function(seed = ? integer, train_data_fraction = ? double) {
  ...
  A_rand <- A_raw[order(runif(n)),] # shuffling data

  n_train <- as.integer(n * train_data_fraction) # `train_data_fration` - training, other - testing
  A_train <- A_rand[1:n_train,]
  A_test <- A_rand[(n_train + 1):n,]
  ...
  return(table(A_predicted, A_test$V10) / as.integer(n * (1 - train_data_fraction)))
}

bayes_try_fraction <- function(train_data_fraction = ? double) {
  per_seed_stats <- data.frame(matrix(ncol = length(per_seed_stat_names), nrow = 0, dimnames = list(NULL, per_seed_stat_names)))
  for (seed in seeds) {
    per_seed_stats[nrow(per_seed_stats) + 1,] <- c(seed, bayes_tic_tac_toe(seed, fraction))
  }
...
for (fraction in c(0.5, 0.6, 0.7, 0.8, 0.9)) {
  per_seed_stats <- bayes_try_fraction(fraction)
...
ggplot(stats_summary, aes(x = fraction, y = stat_mean, colour = stat_name)) +
  ...
  ylab("true negatives") +
  ggtitle("Average behaviour on test data with confidence intervals")
